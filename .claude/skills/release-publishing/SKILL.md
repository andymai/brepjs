...
- **`brepjs@20.0.0` and `create-brepjs@0.4.3` are burned version numbers.** Both were unpublished on 2026-09-21 (a no-op root major cut by the stacked-PR attribution quirk, see `publish-pipeline.md`), and npm never allows an unpublished version to be republished. The manifest was rewound to brepjs 19.0.5, so release-please's next root major would compute 20.0.0 and the publish would fail: put `Release-As: 21.0.0` in the footer of the first intentionally breaking root commit. create-brepjs moved straight to 0.4.4, so it has no trap.
