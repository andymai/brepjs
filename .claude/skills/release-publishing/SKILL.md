...
| Root `exclude-paths` are `apps` and every `packages/*` workspace, so a satellite-only commit never bumps root. A breaking commit touching root `docs/`, `README.md`, `scripts/`, or `.github/` still majors root. | Every non-excluded path attributes to the root component.                                                                                                          | `release-please-config.json:8-21` |
