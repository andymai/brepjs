...
`apps`, `tests` and every `packages/*` workspace are listed in the root component's `exclude-paths`, so a commit that touches only satellite paths or the root test suite does not bump the root library version. A managed satellite such as `packages/brepjs-bim` therefore appears in _both_ the managed list (its own component) and the root's `exclude-paths`: bim-only commits release bim alone, and a satellite release no longer drags a root `brepjs` release with it. Root still attributes every other repo path (`src/`, `docs/`, `scripts/`, `.github/`), so a `!` on a docs or tooling commit majors root. A new `packages/*` directory must be added to the root `exclude-paths` list, or its commits ride the root version again.

release-please attributes a squash commit by the **pull request's** changed-file list (the three-dot diff against the merge base), not by the files in the squash commit itself. A stacked PR whose base PR has already merged still lists the base's files, so a satellite PR that carried a prerequisite touching a non-excluded root path is attributed to root even though the squash commit is satellite-only. #2328 (bim-only squash) cut brepjs 20.0.0 this way because its PR diff still carried #2327's `tests/` files. Before merging a stacked PR, check `gh pr diff <n> --name-only` for paths outside the exclude list.
