...
8. **`boundingBox` is borrowed**: `Curve2D.boundingBox` caches its `BoundingBox2d` and disposes it with the curve, so a box read from a curve must not outlive it. Clone the curve first if you need an independent box. The same holds for `Blueprint`, `Blueprints`, `CompoundBlueprint` and `Drawing`.
9. **Disposal cascades**: deleting a `Drawing`, `Blueprints` or `CompoundBlueprint` deletes its child blueprints and their curves. Containers built from another container's children (e.g. `new Blueprints(other.blueprints.slice(1))`) share those objects; `clone()` first if the two need independent lifetimes.
