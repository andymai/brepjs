...
| `types.ts`          | Type aliases (`OpenCascadeInstance` = `any`, `OcShape` = `TopoDS_Shape`, `OcType` = `any`), `ShapeType` union, `BooleanOptions`, `MeshOptions`, `KernelMeshResult`, `KernelAdapter` interface |
...
3. **Dynamic types** — `OcShape` is typed as `TopoDS_Shape` from brepjs-opencascade bindings; `OcType`/`OpenCascadeInstance` remain `any` for flexibility
