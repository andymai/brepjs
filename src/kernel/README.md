...
| `occtAdapter.ts`  | `OCCTAdapter` class implementing ~70 methods: boolean ops, shape construction, extrusion/sweep/loft, modification (fillet/chamfer/shell/offset), meshing (C++ bulk or JS fallback), topology iteration with hash deduplication. Delegates to operation modules below. |
