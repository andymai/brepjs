...
- **Select edges/faces; don't fillet/chamfer everything.** `fillet(solid, radius)` (no edge list) rounds EVERY edge and frequently fails (`FILLET_FAILED`). Pass an edge list: `edgeFinder().inDirection('Z').findAll(solid)`. Note `inDirection` matches BOTH ± orientations; discriminate a single face/edge by position with `.when(f => getBounds(f).zMax > t)` — `getBounds` is its own import from `'brepjs'`. Finders take a **direction** (`'X'`/`'Y'`/`'Z'`/a `Vec3`), never a plane: a top face is `faceFinder().inDirection('Z')` or `.parallelTo('Z')`; **`parallelTo('XY')` fails `--check`** (`TS2345`) even though the bundled `llms-full.txt` shows it — a face parallel to the XY plane has a `Z` normal, so pass `'Z'`. (See `references/modifiers.md`.)
