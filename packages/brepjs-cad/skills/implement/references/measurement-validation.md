...
- **Snapshot presets follow the model axes** (Z-up): `top` looks down -Z at the +Z face, `front` views from -Y, `right` from +X, `iso` from (+X, -Y, +Z). In `top.png`, +X runs left and +Y runs down. Read scale from the burned-in bbox and confirm exact dimensions against `bounds`.
