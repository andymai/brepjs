...
Civil-semantic wall and railing routes require `bodyEvaluator` (or `proxyEvaluator` as a compatibility fallback) to verify their authored Product Bodies. Without it, projection returns `FAMILIES_PRODUCT_BODY_EVALUATOR_REQUIRED` rather than a parametric envelope. Conventional archetype walls and railings stay specification-authoritative. Registered wall openings are applied first. A coincident authored Body keeps editable parametric IFC; a compound, voided, or otherwise different Body remains a typed Wall or Railing and exports all of its items as tessellation. Other typed civil routes continue to use the reference Families' semantic envelope dimensions.
