...
export default funnel();`,
  },
  {
    ...
    label: 'Measuring Cup',
    description:
      'Volume-sized measuring cup with a thin-wall cavity and a flat hang-hole handle.',
    code: `import {
  convexHull,
  cut,
  cylinder,
...
// Parametric measuring cup with a flat side handle.
//
// The cup is sized by VOLUME: an open cylinder whose inner cavity holds
// exactly \`volumeMl\` millilitres. Following the classic "equilateral" cup
// proportion, the inner height equals the inner diameter, so for a target
// volume V (mm^3) the inner diameter d solves V = (pi/4) * d^2 * d, giving
// d = (4V / pi)^(1/3). The wall and floor are ~1 mm, and a flat 3 mm-thick
// handle sweeps out from the rim with a hang hole at its tip.
function measuringCup(volumeMl = 50, handleLength = 50) {
  const volumeMm3 = volumeMl * 1000;

  // Inner cavity: equilateral cylinder (height == diameter).
  const innerDia = Math.cbrt((4 * volumeMm3) / Math.PI);
  const innerR = innerDia / 2;
  const innerH = innerDia;

  const wall = 1; // side + floor thickness
  const outerR = innerR + wall;
  const outerH = innerH + wall; // floor 1 mm, rim 1 mm above fill line

  // Top-rim fillet: must stay well under the wall thickness or the round can't
  // fit between the inner and outer rim circles, so cap it at ~40% of the wall.
  const rimRound = wall * 0.4;

  // --- Cup body: solid billet, then bore out the cavity ---------------------
  // Outer wall stands on the XY plane (base at z = 0).
  const outerBody = cylinder(outerR, outerH, { at: [0, 0, 0] });
  // Cavity sits on a 1 mm floor and opens through the top (over-bored 1 mm so
  // the cut breaks cleanly through the rim plane).
  const cavity = cylinder(innerR, innerH + 1, { at: [0, 0, wall] });
  let body = unwrap(cut(outerBody, cavity));

  // Round every edge for a soft turned-cup look. With a small radius this
  // softens the rim, base and cavity lip uniformly without a fragile finder.
  body = unwrap(fillet(body, rimRound));

  // --- Handle: a flat 3 mm slab extending out along +Y ----------------------
  const handleThick = 3;
  const barWidth = 10; // handle bar width
  const tipR = 5; // rounded end radius
  const holeR = 3; // hang-hole radius

  // Neck: blend the cup wall into the start of the handle bar with a convex
  // hull, so the slab grows organically out of the cylinder instead of butting
  // against it. Hull an arc of points on the cup wall to a rectangle at the
  // bar root.
  const barStartY = outerR + 8;
  const neckPts: [number, number, number][] = [
    // arc of points hugging the cup's outer wall on the +Y side
    [outerR * Math.sin(-0.6), outerR * Math.cos(-0.6), 0],
    [outerR * Math.sin(0.6), outerR * Math.cos(0.6), 0],
    [outerR * Math.sin(-0.6), outerR * Math.cos(-0.6), handleThick],
    [outerR * Math.sin(0.6), outerR * Math.cos(0.6), handleThick],
    // bar root rectangle
    [-barWidth / 2, barStartY, 0],
    [barWidth / 2, barStartY, 0],
    [-barWidth / 2, barStartY, handleThick],
    [barWidth / 2, barStartY, handleThick],
  ];
  const neck = unwrap(convexHull(neckPts));

  // Bar: straight slab from the neck root out toward the rounded tip.
  const tipCenterY = outerR + handleLength; // centre of the rounded end
  const barPts: [number, number, number][] = [
    [-barWidth / 2, barStartY, 0],
    [barWidth / 2, barStartY, 0],
    [-barWidth / 2, barStartY, handleThick],
    [barWidth / 2, barStartY, handleThick],
    [-barWidth / 2, tipCenterY, 0],
    [barWidth / 2, tipCenterY, 0],
    [-barWidth / 2, tipCenterY, handleThick],
    [barWidth / 2, tipCenterY, handleThick],
  ];
  const bar = unwrap(convexHull(barPts));

  // Rounded tip: a disc capping the bar end.
  const tip = cylinder(tipR, handleThick, { at: [0, tipCenterY, 0] });

  let handle = unwrap(fuseAll([neck, bar, tip]));

  // Hang hole punched through the tip.
  const hole = cylinder(holeR, handleThick + 2, { at: [0, tipCenterY, -1] });
  handle = unwrap(cut(handle, hole));

  // --- Weld handle to cup ---------------------------------------------------
  return unwrap(fuse(body, handle));
}
...
export default measuringCup();`,
  ...
  {
    id: 'pegboard-shelf',
    label: 'Pegboard Shelf Bracket',
    description:
      'Honeycomb-perforated shelf plate on two angled pegboard-hook support legs.',
    code: `// Source: ShelfMaker (clean-room reimplementation of the recognizable form)
import {
  box,
  cut,
  cutAll,
...
//
// The recognizable form: a rounded rectangular shelf plate perforated with a
// honeycomb (hexagon) lattice, carried by two angled triangular support legs.
// Each leg ends in a vertical strip that hooks onto a 20 mm-pitch pegboard
// via a column of peg tabs. We model the assembly centred on the origin so
// the plate, legs, and pegs all share one coordinate frame.
function pegboardShelf(
  shelfWidth = 160, // span across the board (left<->right)
  shelfDepth = 100, // how far the shelf sticks out from the board
  thickness = 5, // shelf-plate thickness
  hexInradius = 4, // honeycomb half-width across flats
  hexGap = 1.5, // wall between adjacent hex cells
  numPegs = 3, // peg tabs per support leg
) {
  const cornerR = 5; // rounded plate corners, soft pegboard-friendly look
  const pegPitch = 20; // pegboard hole spacing, vertical
  const legInset = 10; // legs stop short of the front edge for a clean look

  // ---- Shelf plate -------------------------------------------------------
  // Rounded rectangle extruded up by \`thickness\`, base on z = 0.
  const plate = sketchRoundedRectangle(shelfWidth, shelfDepth, cornerR).extrude(
    thickness,
  );

  // ---- Honeycomb perforations -------------------------------------------
  // Lay a staggered hex grid over the plate and cut each cell straight
  // through. Outer (vertex) radius derived from the inradius across flats.
  const hexR = hexInradius / Math.cos(Math.PI / 6); // circumradius
  const stepX = hexInradius * 2 + hexGap; // column pitch (flats + gap)
  const rowOff = stepX * (Math.sqrt(3) / 2); // hex row half-step
  const margin = cornerR + hexInradius + 1; // keep cells inside the rounded rim
  const halfW = shelfWidth / 2 - margin;
  const halfD = shelfDepth / 2 - margin;

  const holes = [];
  const cols = Math.floor((halfW * 2) / stepX);
  const rows = Math.floor((halfD * 2) / (rowOff * 2));
  for (let i = 0; i <= cols; i++) {
    for (let j = 0; j <= rows; j++) {
      // Two interleaved lattices give the brick-staggered honeycomb.
      for (const [ox, oy] of [
        [0, 0],
        [stepX / 2, rowOff],
      ]) {
  ...
  const perforated = unwrap(cutAll(plate, holes));

  // ---- Support legs ------------------------------------------------------
  // Each leg is a right-triangle gusset (a wedge) under the plate plus a
  // vertical mounting strip carrying the peg tabs. Build one leg in a local
  // frame whose back face (x = 0) is the pegboard plane, then place a copy at
  // each side of the plate.
  const legThick = 8; // beam width of the gusset
  const stripW = 12; // vertical mount-strip width
  const legRun = shelfDepth - legInset; // forward reach of the gusset
  const legRise = (numPegs - 1) * pegPitch + 14; // drop to the lowest peg

  // The board plane sits at the shelf's back edge; the shelf sticks out in +Y.
  const boardY = -shelfDepth / 2;

  function makeLeg() {
    // Triangular gusset: a box sliced by a diagonal plane so it tapers from
    // full height at the board to nothing at the front. Local +Y runs forward
    // from the board (the stick-out direction); the gusset hangs below the
    // plate (negative z) and is thin along X (the leg-spacing axis).
    const gussetBlock = box(legThick, legRun, legRise, {
      at: [0, boardY + legRun / 2, -legRise / 2],
    });
    // Slicing wedge: a big box rotated about X to shave the underside diagonal,
    // tapering the gusset from full height at the board to nothing at the front.
    const slope = (Math.atan2(legRise, legRun) * 180) / Math.PI;
    const knife = translate(
      rotate(box(legThick * 2, legRun * 2, legRise * 2, { at: [0, 0, 0] }), -slope, {
        axis: [1, 0, 0],
        at: [0, 0, 0],
      }),
      [0, boardY, -legRise],
    );
    const gusset = unwrap(cut(gussetBlock, knife));

    // Vertical mount strip flush against the board (y = boardY plane).
    const stripH = legRise + 14;
    const strip = box(legThick, stripW, stripH, {
      at: [0, boardY + stripW / 2, -stripH / 2 + 4],
    });

    // Peg tabs: small hooks protruding toward the board (-y), one per row.
    const tabs = [];
    for (let p = 0; p < numPegs; p++) {
      const z = -p * pegPitch;
      tabs.push(box(4, 4, 5, { at: [0, boardY - 2, z] }));
    }

    return unwrap(fuseAll([gusset, strip, ...tabs]));
  }

  const legBase = makeLeg();
  // One leg at each side, just inboard of the plate edge.
  const legL = translate(legBase, [-shelfWidth / 2 + legThick / 2, 0, 0]);
  const legR = translate(legBase, [shelfWidth / 2 - legThick / 2, 0, 0]);

  // Plate rides on top of the legs (their tops sit at z = +4).
  const plateOnTop = translate(perforated, [0, 0, 4]);

  return unwrap(fuseAll([plateOnTop, legL, legR]));
}

export default pegboardShelf();`,
  },
  {
    id: 'rugged-case',
    label: 'Rugged Case (waterproof box body)',
    description:
      'Pelican-style rugged container body: rounded-corner shell with sealing rim, side ribs, and back hinge knuckles.',
    code: `import {
  cut,
  cylinder,
  fuseAll,
...
// Parametric rugged waterproof case (Pelican-style lower body).
//
// The recognizable form is a thick-walled rounded-corner box with a raised
// sealing rim around the open top, external reinforcing ribs banding the side
// walls, and a pair of hinge knuckles along the back edge.
//
// Defaults track the reference dimensions: a 140 x 80 mm internal footprint,
// 75 mm tall, a 12 mm corner radius, ~2.5 mm walls and a 16 mm sealing rim.
// The cavity is made with a boolean cut (outer solid minus inner solid) rather
// than a thin-wall shell, which is the robust route for a multi-feature part.
function ruggedCase(
  length = 140,
  width = 80,
  height = 75,
  ...
  // --- Outer envelope ---------------------------------------------------
  // Wall thickness is added on each side so the carved cavity keeps the
  // nominal length x width internally. Profile is a rounded rectangle.
  const outerL = length + 2 * wall;
  const outerW = width + 2 * wall;
  const body = sketchRoundedRectangle(outerL, outerW, corner).extrude(height);

  // --- Sealing rim ------------------------------------------------------
  // A wider, taller collar around the open top that the lid would clamp onto.
  // It is the widest feature at the top of the case.
  const rim = sketchRoundedRectangle(
    outerL + 2 * wall,
    outerW + 2 * wall,
    corner + wall,
    { origin: [0, 0, height - rimHeight] },
  ).extrude(rimHeight);

  // --- External reinforcing ribs ----------------------------------------
  // Slender proud pads straddling both long walls at the +/- length/3 stations
  // (echoing the reference's stiffening ribs). The part overlapping the wall
  // fuses in; the rest stands proud as a raised band. They stop below the rim.
  const ribProud = 1.5; // stand-off beyond the wall
  const ribPad = 5; // how far the pad bites into the wall, for a clean fuse
  const ribH = height - rimHeight; // ribs span the lower body only
  const ribStations = [length / 3, -length / 3];
  const ribs = ribStations.flatMap((x) => {
    const padDepth = ribProud + ribPad;
    const yOut = outerW / 2 + ribProud - padDepth / 2;
    return [yOut, -yOut].map((y) =>
      sketchRoundedRectangle(ribThickness, padDepth, ribThickness / 2.5, {
        origin: [x, y, 0],
      }).extrude(ribH),
    );
  });
  ...
  // --- Hinge knuckles ---------------------------------------------------
  // Two short cylinders along the back wall (+Y) whose common axis runs along
  // X -- the lid pin would thread through these. Built as a +Z cylinder then
  // rotated -90 deg about X so it lies horizontal against the back face.
  const knuckleR = 5;
  const knuckleLen = 14;
  const backY = outerW / 2;
  const knuckleZ = height - rimHeight - knuckleR;
  const knuckles = ribStations.map((x) =>
    rotate(cylinder(knuckleR, knuckleLen, { at: [x, backY, knuckleZ] }), -90, {
      axis: [1, 0, 0],
      at: [x, backY, knuckleZ],
    }),
  );

  // --- Assemble the solid mass -----------------------------------------
  const solid = unwrap(fuseAll([body, rim, ...ribs, ...knuckles]));

  // --- Carve the cavity -------------------------------------------------
  // An inner rounded box, started above the floor and run out through the open
  // top, leaves a wall-thick shell with a flat floor. The cavity reaches up
  // into the rim so the rim becomes a clamping lip, not a solid cap.
  const cavity = sketchRoundedRectangle(
    length,
    width,
    corner - wall,
    { origin: [0, 0, wall] },
  ).extrude(height + rimHeight); // overshoot the top so it cuts clean through

  // The rounded-rect profile already gives soft vertical corners, so the carved
  // cavity is the finished part -- no fragile edge fillet over the thin walls.
  const finished = unwrap(cut(solid, cavity));

  return finished;
}

export default ruggedCase();`,
  },
];
