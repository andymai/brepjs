/**
 * Shared Open Graph card machinery: the brand card template and the
 * headless-Chrome render pipeline. Consumers enumerate their own pages and
 * supply the copy: scripts/gen-og-docs.mjs (docs pages) and
 * apps/playground/scripts/genOgExamples.ts (playground examples).
 */
...
export const headlineSize = (text) => {
  const n = text.length;
  if (n <= 16) return 66;
  if (n <= 26) return 56;
  if (n <= 38) return 48;
...
// Signifier is licensed + self-hosted — inline the local woff2 so headless
// Chrome renders the brand face without Google Fonts. Requires the licensed
// files (gitignored; see apps/docs/public/fonts/README.md).
export async function signifierFontCss(fontsDir) {
  const fontFile = (f) => resolve(fontsDir, f);
  for (const f of ['signifier-regular.woff2', 'signifier-italic.woff2', 'signifier-medium.woff2']) {
    if (!existsSync(fontFile(f)))
      throw new Error(
        `Missing ${f} in ${fontsDir} — license Signifier and drop the woff2 there to regenerate OG cards.`
      );
  }
...
export const defaultVisualHtml = () =>
  `<div class="geo-glow"></div><div class="geo">${orthoscheme}</div>`;
...
export function cardHtml({
  ...
  subhead,
  contentWidth = 700,
  visualHtml = defaultVisualHtml(),
  footHtml = '<span class="dom">brepjs.dev</span>',
}) {
  return `<!doctype html>
<html>
<head>
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .content { position: absolute; left: 72px; top: 0; bottom: 0; width: ${contentWidth}px;
  ...
  .thumb { position: absolute; right: 56px; top: 50%; width: 400px; height: 400px;
    transform: translateY(-50%); border-radius: 20px; overflow: hidden;
    border: 1px solid rgba(255, 255, 255, 0.14);
    box-shadow: 0 26px 70px rgba(3, 176, 173, 0.22), 0 6px 24px rgba(0, 0, 0, 0.5); }
...
// back to puppeteer's bundled browser so card generation also works on macOS
// and Chromium-only Linux.
export async function createOgRenderer() {
  const systemChrome = [
  ...
  // Cards render at 2x then downscale (crisp edges) and palette-quantize.
