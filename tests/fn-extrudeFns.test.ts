...
describe('sweep', () => {
  it('sweeps a circle along a line', () => {
    ...
    const profile = castShape(circle.wire.wrapped) as Wire;
    const e = makeLine([0, 0, 0], [0, 0, 20]);
    const spine = castShape(unwrap(assembleWire([e])).wrapped) as Wire;
...
describe('complexExtrude', () => {
  it('extrudes with linear profile', () => {
    const circle = sketchCircle(5);
    const wire = castShape(circle.wire.wrapped) as Wire;
...
describe('twistExtrude', () => {
  it('extrudes with twist', () => {
    ...
    const wire = castShape(rect.wire.wrapped) as Wire;
